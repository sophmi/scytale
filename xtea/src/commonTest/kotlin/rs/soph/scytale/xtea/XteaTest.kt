...
class XteaTest {
	...
	fun `inputs of zeroes match`() {
		for ((index, ciphertext) in EMPTY_INPUT_KEYS.withIndex()) {
			...
			val ints = IntArray(Xtea.BLOCK_SIZE_BYTES / Int.SIZE_BYTES)
			assertXtea(ints, key, ciphertext)
		}
	}

	@Test
	fun `inputs of zeroes match when offset`() {
		for ((index, ciphertext) in EMPTY_INPUT_KEYS.withIndex()) {
			val key = IntArray(Xtea.KEY_SIZE_INTS)
			val int = index / Int.SIZE_BITS
			key[int] = (-0x8000_0000 ushr index % Int.SIZE_BITS)

			val byteCiphertext = "000000$ciphertext"
			val bytes = ByteArray(3 + Xtea.BLOCK_SIZE_BYTES)
			assertXtea(bytes, key, byteCiphertext, offset = 3)

			val intCiphertext = "0000000000000000$ciphertext"
			val ints = IntArray(2 + Xtea.BLOCK_SIZE_BYTES / Int.SIZE_BYTES)
			assertXtea(ints, key, intCiphertext, offset = 2)
		}
	}

	@Test
	fun `inputs of zeroes match when offset with fixed length`() {
		for ((index, ciphertext) in EMPTY_INPUT_KEYS.withIndex()) {
			val key = IntArray(Xtea.KEY_SIZE_INTS)
			val int = index / Int.SIZE_BITS
			key[int] = (-0x8000_0000 ushr index % Int.SIZE_BITS)

			val byteCiphertext = "000000${ciphertext}0000"
			val bytes = ByteArray(5 + Xtea.BLOCK_SIZE_BYTES)
			assertXtea(bytes, key, byteCiphertext, offset = 3, length = Xtea.BLOCK_SIZE_BYTES)

			val intCiphertext = "00000000${ciphertext}0000000000000000"
			val ints = IntArray(3 + Xtea.BLOCK_SIZE_BYTES / Int.SIZE_BYTES)
			assertXtea(ints, key, intCiphertext, offset = 1, length = Xtea.BLOCK_SIZE_BYTES / Int.SIZE_BYTES)
		}
	}
	...
	@Test
	fun `inputs of repeated bytes match`() {
		for ((b, ciphertext) in REPEATED_BYTES.withIndex()) {
			val key = IntArray(Xtea.KEY_SIZE_INTS) { (b shl 24) or (b shl 16) or (b shl 8) or b }

			val bytes = ByteArray(Xtea.BLOCK_SIZE_BYTES) { b.toByte() }
			assertXtea(bytes, key, ciphertext)

			val ints = IntArray(Xtea.BLOCK_SIZE_BYTES / Int.SIZE_BYTES) { (b shl 24) or (b shl 16) or (b shl 8) or b }
			assertXtea(ints, key, ciphertext)
		}
	}
	...
	private fun assertXtea(
		...
		Xtea.encipher(input, key, offset, length)
		assertContentEquals(ciphertext.hexToBytes(), input, "Failed to encipher: ${bytes.toUpperHexString()}")

		Xtea.decipher(input, key, offset, length)
		assertContentEquals(bytes, input, "Failed to decipher: $ciphertext")
	}
	...
	private fun assertXtea(
		...
		val input = ints.copyOf()
		Xtea.encipher(input, key, offset, length)
		assertContentEquals(ciphertext.hexToInts(), input, "Failed to encipher: ${ints.toUpperHexString()}")

		Xtea.decipher(input, key, offset, length)
		assertContentEquals(ints, input, "Failed to decipher: $ciphertext")
	}

	private companion object {

		/**
		 * Ciphertexts for an input with a single set bit and a key of 0, ordered from highest bit set to lowest, i.e.
		...
		/**
		 * Ciphertexts for an empty input with a single bit set in the key, i.e.
		 * ```EMPTY_INPUT_KEYS[0] == encipher(0, key = 0x80000000_00000000_00000000_00000000)```.
		 */
		private val EMPTY_INPUT_KEYS = listOf(
		...
		/**
		 * Ciphertexts for inputs and keys of the same byte repeated, ordered by that same byte value, i.e.
