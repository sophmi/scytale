...
public class Whirlpool {
	...
	private fun encipher(input: ByteArray, start: Int, end: Int): Int {
		...
		if (this.offset >= 0) {
			...
			nextBlock += BLOCK_SIZE_BYTES - this.offset
			input.copyInto(buffer, this.offset, start, nextBlock)
			encipherBuffer()
		}
	...
	private fun encipherDirect(input: ByteArray, start: Int, end: Int): Int {
		val excess = (end - start) % BLOCK_SIZE_BYTES
		for (offset in start until (end - excess) step BLOCK_SIZE_BYTES) {
